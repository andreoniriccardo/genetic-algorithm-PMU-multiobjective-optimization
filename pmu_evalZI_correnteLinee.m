function [f_eval, err] = pmu_evalZI_correnteLinee(chrom)    
    ...
    % Numero di misure di tensione ai nodi (è pari al numero di PMU)
    tot_meas = nnz(chrom);
    % Numero di misure di corrente iniettata netta ai nodi (è pari al
    ...
    measured_branches_logical = 1*(measured_branches ~= 0); % Considero che la corrente di una linea può essere 
                                                            % misurata al
                                                            % massimo una
                                                            % volta           
    ...
    % L'elemento V_meas_R(i,j) è la j-esima misura della parte reale del
    ...
    % L'elemento I_meas_R(i,j) è la j-esima misura della parte reale della
    ...
    if f2 <= max_f2(gen)
        % Definisco il vettore u, dove u_j = 1 se il bus j è osservato da una
        % PMU, ed è 0 altrimenti
        u = O>=1;

        % Definisco il vettore v, dove:
        % v_j = 0 se il bus j non è ZI
        % v_j = 1 se il bus j è ZI e A_j*u >= omega_j - 1
        v = zeros(G,1);
        for j = 1:numel(ZIbus)
        ...
        for i = 1:G
            % Creo la matrice Ap che è uguale alla matrice Incidence ma con
            % la colonna i-esima pari al vettore nullo
            Ap = Incidence;
            Ap(:,i) = zeros(G,1);
            % Data la matrice Ap calcolo il corrispondente vettore di
            % osservabilità Op (a seguito della perdita della PMU i)
            Op = Ap*chrom';
            % Determino u_p:
            up = Op>=1;
